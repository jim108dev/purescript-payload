...
type MethodHandler
  = RequestUrl -> HTTP.Request -> HTTP.Response -> Result RawResponse
...
instance handleablePostRoute ::
  ...
  Handleable (Route "POST" path (Record route)) (Record payload -> Aff handlerRes) basePath baseParams baseGuards guardsSpec (Record allGuards) where
  handle _ _ _ _ route handler allGuards { method, path, query } req res = do
    ...
    let
      (payload :: Record payloadWithEmpty) = to { params, body, query: decodedQuery, guards: guards }
...
instance handleableGetRoute ::
  ...
  Handleable (Route "GET" path (Record route)) (Record payload -> Aff handlerRes) basePath baseParams baseGuards guardsSpec (Record allGuards) where
  handle _ _ _ _ route handler allGuards { method, path, query } req res = do
    guards <- runGuards (Guards :: _ fullGuards) (GuardTypes :: _ (Record guardsSpec)) allGuards {} req
    params <- withExceptT Forward $ except $ decodePath path
    decodedQuery <- withExceptT badRequest $ except $ decodeQuery query
    let
      (payload :: Record payloadWithEmpty) = to { params, query: decodedQuery, guards }
    mkResponse (Proxy :: _ docRoute) (Proxy :: _ res) (handler (omitEmpty payload))
    where
    badRequest :: String -> Failure
    badRequest s = Error $ Resp.badRequest (StringBody s)
...
instance handleableHeadRoute ::
  ...
  Handleable (Route "HEAD" path (Record route)) (Record payload -> Aff handlerRes) basePath baseParams baseGuards guardsSpec (Record allGuards) where
  handle _ _ _ _ route handler allGuards { method, path, query } req res = do
    guards <- runGuards (Guards :: _ fullGuards) (GuardTypes :: _ (Record guardsSpec)) allGuards {} req
    params <- withExceptT Forward $ except $ decodePath path
    decodedQuery <- withExceptT badRequest $ except $ decodeQuery query
...
instance handleablePutRoute ::
  ...
  Handleable (Route "PUT" path (Record route)) (Record payload -> Aff handlerRes) basePath baseParams baseGuards guardsSpec (Record allGuards) where
  handle _ _ _ _ route handler allGuards { method, path, query } req res = do
    guards <- runGuards (Guards :: _ fullGuards) (GuardTypes :: _ (Record guardsSpec)) allGuards {} req
    params <- withExceptT Forward $ except $ decodePath path
    decodedQuery <- withExceptT badRequest $ except $ decodeQuery query
    ...
    body <- withExceptT badRequest $ except $ (decodeOptionalBody bodyStr :: Either String body)
    let
      (payload :: Record payloadWithEmpty) = to { params, body, query: decodedQuery, guards }
    mkResponse (Proxy :: _ docRoute) (Proxy :: _ res) (handler (omitEmpty payload))
    where
    badRequest :: String -> Failure
    badRequest s = Error $ Resp.badRequest (StringBody s)
...
instance handleableDeleteRoute ::
  ...
  Handleable (Route "DELETE" path (Record route)) (Record payload -> Aff handlerRes) basePath baseParams baseGuards guardsSpec (Record allGuards) where
  handle _ _ _ _ route handler allGuards { method, path, query } req res = do
    guards <- runGuards (Guards :: _ fullGuards) (GuardTypes :: _ (Record guardsSpec)) allGuards {} req
    params <- withExceptT Forward $ except $ decodePath path
    decodedQuery <- withExceptT badRequest $ except $ decodeQuery query
...
instance handleableOptionsRoute ::
  ...
  Handleable (Route "OPTIONS" path (Record route)) (Record payload -> Aff handlerRes) basePath baseParams baseGuards guardsSpec (Record allGuards) where
  handle _ _ _ _ route handler allGuards { method, path, query } req res = do
    guards <- runGuards (Guards :: _ fullGuards) (GuardTypes :: _ (Record guardsSpec)) allGuards {} req
    params <- withExceptT Forward $ except $ decodePath path
    decodedQuery <- withExceptT badRequest $ except $ decodeQuery query
...
mkResponse ::
  forall handlerRes res docRoute.
  Resp.ToSpecResponse docRoute handlerRes res =>
  Resp.EncodeResponse res =>
...
readBody_ req cb = do
  buffer <- Ref.new ""
  let
    inputStream = HTTP.requestAsStream req
  let
    handleData str = (flip Ref.modify_) buffer (_ <> str)
  let
    handleEnd = Ref.read buffer >>= returnBody
